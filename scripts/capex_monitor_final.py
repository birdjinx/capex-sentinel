...
class CapexMonitor:
    ...
    def load_sec_signals(self):
        """SEC CAPEX 신호 데이터 로드 + 빅테크 CAPEX 트렌드 + 괴리도 계산"""
        try:
            if os.path.exists('data_sec_capex.json'):
                with open('data_sec_capex.json', 'r', encoding='utf-8') as f:
                    ...
                    # 🎯 괴리도 데이터 구성 (대시보드용)
                    gap_data = self.calculate_gap_data(capex_by_company)
                    self.data['components']['gap_data'] = gap_data
    ...
    def calculate_gap_data(self, capex_by_company):
        """각 기업별 괴리도 데이터 생성 (대시보드 표시용)"""
        gap_summary = {
            'current_gap': 0,
            'companies': {}
        }
        ...
        for ticker, weight in weights.items():
            if ticker in capex_by_company:
                capex_qoq = capex_by_company[ticker].get('latest_qoq', 0)
                # 매출은 간단히 추정 (실제로는 대시보드에서 입력한 값 사용)
                # 여기서는 시뮬레이션 값 사용
                estimated_revenue_qoq = capex_qoq * 0.6  # 대략 60% 수준으로 추정
                gap = capex_qoq - estimated_revenue_qoq
                
                gap_summary['companies'][ticker] = {
                    'capex_qoq': round(capex_qoq, 1),
                    'gap': round(gap, 1)
                }
                
                weighted_gap += gap * weight
        
        gap_summary['current_gap'] = round(weighted_gap, 1)
        return gap_summary
    ...
    def run(self):
        ...
        self.load_sec_signals()  # ← 이제 빅테크 CAPEX 트렌드도 계산함
